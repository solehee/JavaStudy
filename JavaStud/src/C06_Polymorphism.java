...
public class C06_Polymorphism {
	...
	*/
	public static void main(String[] args) {
		...
		// # 다운 캐스팅이 가능한 경우
		// - 원래 자식 타입이였던 인스턴스가 업캐스팅되어 있는 경우, 다시 다운캐스팅 될 수 있다
		System.out.println(fruit01);
		pine01 = (Pineapple)fruit01;// 다운캐스팅
		
		// # 업캐스팅 한 인스턴스 사용해보기
		Fruit f = new Pineapple();
		Fruit f2 = new DragonFruit();
		
		// 업캐스팅이 된 상태에서도 오버라이드 된 메서드를 사용하면 자식의 기능을 사용한다
		f.eat();
		
		// 업캐스팅 된 상태에서는 본래 클래스가 갖고있던 기능을 잠시 잊어버린다.
		// (업캐스팅된 상태에서 Pineapple에만 존재하는 기능은 사용할 수 없다)
		// f.smash();
...
class Pineapple extends Fruit{
	...
	void smash() {
		System.out.println("파인애플을 세게 내려쳤습니다.");
	}
}
