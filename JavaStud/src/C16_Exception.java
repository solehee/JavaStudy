...
public class C16_Exception {
	...
	/* 
		...
		// 문법상 문제는 없지만 컴파일하면 에러뜨는 것
		- 자바의 문법을 틀리는 것은 예외가 아니라 컴파일 에러라고 부른다(오타 등)
		- 프로그래머가 발생가능한 예외를 미리 예상하고 대비해두는 것을 예외처리라고 부른다
		- 예외처리의 기본 동작은 프로그램 강제 종료이지만 프로그래머가 다른 조치를 취한다면
		  프로그램을 계속해서 진행할 수 있다
		...
		- catch문에는 발생한 예외에 대한 정보를 담고 있는 예외 인스턴스가 전달된다
	...
	*/
	...
	public static void main(String[] args) {
		...
		try{
			...
			e.printStackTrace();
			System.out.println("말도안되는 계산을 하셨네요. 다음부터는 그러지마세요.");
