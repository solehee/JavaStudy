...
public class D09_CollectEmail {
	/*
	 	다음 문자열에서 올바른 형식의 이메일만 적절한 컬랙션에 저장해보세요
	...
	*/
	...
	public static void main(String[] args) {
		Pattern check = Pattern.compile("\\w{1,64}@\\w{2,63}([.]\\w{2,63})+");
		Matcher emailCheck = check.matcher(email);
		
		
		ArrayList<String> emailList = new ArrayList<>();
		HashSet<String> maind = new HashSet<>();
		while(emailCheck.find()) {
			emailList.add(emailCheck.group());					
			
		}	
		System.out.println(emailList.size());
		maind.addAll(emailList);
		
		maind.retainAll(emailList);
		System.out.println(maind.size());
		
		emailList.removeAll(maind);
		System.out.println(emailList);
		String dfd ="";
		for(String dn : emailList) {
			dfd+=dn;
		}
