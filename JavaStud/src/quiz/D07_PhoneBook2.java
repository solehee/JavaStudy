...
/*
	...
	4. 등록된 모든 번호를 선택한 항목(Info 필드) 기준으로 오름/내림차순으로 보여주는 메서드
	5. 전화번호의 일부를 입력하면 일치하는 모든 정보가 출력되는 메서드
		(아무것도 전달하지 않을 시 기존 순서로 출력)
*/
public class D07_PhoneBook2 {
	...
	public boolean addGroup(String groupName) {
		if(phonebook.containsKey(groupName)) {
			...
			System.out.printf("새그룹 \"%s\"가 추가되었습니다.\n",groupName);
			return true;			
		}
	}
		...
		for(String groupName : phonebook.keySet()) {
			...
			for(Entry<String, Info> entry : group.entrySet()) {
				if(entry.getValue().name.contains(name)) {
					// String a.contains ()안에 들어가잇는 내용이 a에 포함되어있는지 확인
	...
	public void show(int column, boolean asc ) {
		...
		for(HashMap<String, Info> group : phonebook.values()) {
			...
			// info 클래스 정의해주면 이렇게 꺼낼 필요가 없다
			//System.out.println(info.getClass().getFields()[2].getName());
			//}
		}
		System.out.println(info_set);
	}
	...
	public static void main(String[] args) {
       ...
       phone.addInfo("친구", new Info("민수", 17, "010-0001-1234"));
		...
		phone.searchByPhoneNumber("11");
		phone.searchByPhoneNumber("234");
		phone.searchByPhoneNumber("0101111");// - 없어도 검색하는 방법은  replace 로 - 없애버림(String)
